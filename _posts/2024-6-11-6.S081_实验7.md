...
#### 上下文切换，就是将原来的线程的上下文保存到context，然后读取新线程的上下文到寄存器中
...
​	在本作业中，您将实现一个[屏障](https://en.wikipedia.org/wiki/Barrier_(computer_science))（Barrier）：应用程序中的一个点，所有参与的线程在此点上必须等待，直到所有其他参与线程也达到该点。您将使用pthread条件变量，这是一种序列协调技术，类似于xv6的`sleep`和`wakeup`。
